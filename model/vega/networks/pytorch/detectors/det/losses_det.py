...
from vega.networks.pytorch.detectors.det_bbox import bbox_overlaps, min_area_square
from vega.networks.pytorch.detectors.det_box_coder import BoxCoder

from vega.networks.pytorch.detectors.overlaps.rbox_overlaps import rbox_overlaps

if not "win32" == platform:
    from vega.networks.pytorch.detectors.overlaps_cuda.rbbox_overlaps  import rbbx_overlaps
...
#         import ipdb;ipdb.set_trace()
        for j in range(batch_size):
            ...
            if not torch.is_tensor(sa):
                # import ipdb;ipdb.set_trace()
                sa = torch.from_numpy(sa).to(regressions.device)
            if var != -1:
                ...
                if not torch.is_tensor(fa):
                    fa = torch.from_numpy(fa).to(regressions.device)
