...
#  core 车道线类别判断
#---------------------------------------------------#
CLASS_LINE_TYPE = ["line-special",
                    "line-single-solid-white",
                     "line-single-solid-yellow",
                     "line-single-dash-white",
                     "line-single-dash-yellow",
                     "line-double-solid-white",
                     "line-double-solid-yellow",
                     "line-double-dash-white",
                     "line-double-dash-yellow"
                     ]

CLASS_ABNORMAL_LINE = [
    "line-fish-bone-dash",
    "line-fish-bone-solid",
    "line-solid-wide",
    "line-solid-dash-white",
    "line-dash-solid-white",
    "line-solid-dash-yellow",
    "line-dash-solid-yellow",
    "line-slow"
]
...
def evaluate_core(*,
    ...
    :type gt_labels: a list of lanes in an image
    :type pr_labels: a list of lanes in an image
    ...
    pr_num = len(pr_lanes)
    hit_num = 0
    hit_num_with_type = np.zeros(len(CLASS_LINE_TYPE))
    gt_num_with_type = np.zeros(len(CLASS_LINE_TYPE))
    ...
    if gt_num > 0 and pr_num > 0:
        ...
        for gt_index, pr_index in zip(*match_index_list):
            ...
            if iou_val > iou_thresh:
                ...
                pred_label = pr_labels[pr_index]
                if gt_label  in CLASS_ABNORMAL_LINE:
                    gt_label = "line-special"

                for idx_type, type_name in enumerate(CLASS_LINE_TYPE):
                    if gt_label == type_name:
                        gt_num_with_type[idx_type]+=1

                        if gt_label == pred_label:
                            hit_num_with_type[idx_type]+=1

    return dict(gt_num=gt_num, pr_num=pr_num, hit_num=hit_num,
                hit_lane_with_type=hit_num_with_type, gt_lane_with_type=gt_num_with_type)
...
class LaneMetricCore(MetricBase):
    ...
    def __call__(self, gt_result, pr_result, *args, **kwargs):
        ...
        for line_spec in predict_spec['Lines']:
            line_pred_type = line_spec["lane_type"]
            if 'score' in line_spec:
                if line_spec['score'] > prob_thresh:
                    # print(line_spec["score"])
                    # print(line_spec["score"].detach().cpu().numpy())
                    line_spec = line_spec['points']
                else:
                    line_spec = []
            if len(line_spec) > 0:
                pr_lanes.append(line_spec)
                pr_labels.append(line_pred_type)
    ...
    def summary(self):
        ...
        # 车道线类别
        if len(self.result_record) > 0:
            hit_num_with_type_total = np.vstack( [result['hit_lane_with_type'] for result in self.result_record] ).sum(axis=0)
            gt_lane_with_type_total = np.vstack( [result['gt_lane_with_type'] for result in self.result_record] ).sum(axis=0)
            # 每个类别准确率
            lane_cls_precison_array = hit_num_with_type_total / (gt_lane_with_type_total.astype(np.float32) + 1e-8)

            hit_num_total = hit_num_with_type_total.sum()
            gt_num_total = gt_lane_with_type_total.sum()
            # 整体准确率
            lane_cls_precison_avg = hit_num_total / (gt_num_total.astype(np.float32) + 1e-8)


        else:
            lane_cls_precison_array = np.zeros(len(CLASS_LINE_TYPE))
            lane_cls_precison_avg = 0.0

        return dict(f1_measure=f1_measure, precision=precision, recall=recall,
                    lane_cls_precison_array=lane_cls_precison_array, lane_cls_precison_avg=lane_cls_precison_avg)
...
class LaneMetric(MetricBase):
    ...
    def summary(self):
        """Summary all record from result cache, and get performance."""
        # print all metric
        metric_iou_thres_list = [0.50, 0.60, 0.70, 0.80, 0.90]
        metric_list = [handlers.summary()[self.method] for handlers in self.metric_handlers]
        lane_cls_precisons_type = [handlers.summary()["lane_cls_precison_array"] for handlers in self.metric_handlers]
        lane_cls_precisons_avg = [handlers.summary()["lane_cls_precison_avg"] for handlers in self.metric_handlers]

        for thre_iou,metric, precision_type,precison_avg in zip(metric_iou_thres_list,metric_list,lane_cls_precisons_type,lane_cls_precisons_avg):
            print("thres_iou: %.2f" % thre_iou)
            print("metric: %.2f" % metric)
            print("lane type classification precision:")
            if precision_type is not None:
                for cls_name,precison in zip(CLASS_LINE_TYPE,precision_type):
                    print(cls_name,": %.2f" % precison)
            print("lane type classification mean precison: %.2f" %precison_avg)
            print("-----------------------------------------")
