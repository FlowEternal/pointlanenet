...
def create_train_subset(data_path,
    ...
    :type data_path: str
    """
    images_list_path = os.path.join(data_path, list_type, 'train.txt')
    ...
    for image_path_spec in images_list:
        if not is_own:
            path_pair_spec = dict(
                ...
                annot_path=os.path.normpath(f'{data_path}/{image_path_spec}'.replace('.jpg', '.lines.json').replace("images","labels"))
            )
            ...
            path_pair_spec = dict(
                image_path=os.path.normpath(f'{data_path}/{image_path_spec}'),
                annot_path=os.path.normpath(f'{data_path}/{image_path_spec}'.replace('.jpg', '.json').replace("images","labels_lane"))
            )
            if with_seg:
                path_pair_spec.update({"annot_path_seg":os.path.normpath(f'{data_path}/{image_path_spec}'.replace('.jpg', '.png').replace("images","labels_segmentation"))})
            if with_detect:
                path_pair_spec.update({"annot_path_detect":os.path.normpath(f'{data_path}/{image_path_spec}'.replace('.jpg', '.txt').replace("images","labels_object"))})
...
def create_valid_subset(data_path,
    ...
    :type data_path: str
    """
    images_list_path = os.path.join(data_path, list_type, 'val.txt')
    images_list = load_lines(images_list_path)
    path_pairs = []
    for image_path_spec in images_list:
        path_pair_spec = dict(
...
def create_test_subset(data_path,
    ...
    :type data_path: str
    """
    images_list_path = os.path.join(data_path, list_type, 'test.txt')
    images_list = load_lines(images_list_path)
    path_pairs = []
    for image_path_spec in images_list:
        if not is_own:
