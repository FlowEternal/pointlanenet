...
CLASS_NORMAL_LINE = ["line-single-solid-white",
...
class PointLaneCodec(object):
    ...
    def __init__(self,
        ...
        self.with_lane_cls = with_lane_cls          # 是否预测车道线类别
        self.lane_cls_num = lane_cls_num            # 车道线多标签数
        self.threshold_lane_type = 0.5              # 车道线类别判断阈值
    ...
    def encode_lane(self, lane_object, org_width, org_height):
        ...
        if self.with_lane_cls:
            type_list = lane_object["Labels"]
            gt_lane_class = np.zeros(shape=(self.feature_height * self.feature_width, self.lane_cls_num),dtype=float)
        else:
            type_list = None
            gt_lane_class = None
        ...
        if len(gt_lanes_list) < 1:
            ...
            for idx,lane in enumerate(lane_set):
                ...
                # 加入分类相关向量
                if self.with_lane_cls:
                    type_name = str(type_list[idx])

                    # 编码车道线多标签类别
                    for tmp_anchor_index in anchor_list:
                        if type_name in CLASS_ABNORMAL_LINE:

                            tmp_value = np.zeros(self.lane_cls_num)
                            tmp_value[0] = 1
                        else:

                            tmp_value = np.zeros(self.lane_cls_num)
                            for idx_cls, name in enumerate(CLASS_NORMAL_LINE):
                                if name == type_name:
                                    tmp_value[idx_cls + 1] = 1
                                    break

                        gt_lane_class[tmp_anchor_index[0] * self.feature_width + tmp_anchor_index[1],0:self.lane_cls_num] =tmp_value
        ...
        return gt_type, gt_loc, gt_lane_class

    @staticmethod
    def decode_lane_type(predict_type_array):
        index_max = np.argmax(predict_type_array)
        if index_max ==0:
            return "line-special"
        else:
            return CLASS_NORMAL_LINE[index_max-1]
    ...
    def decode_lane(self,
        ...
        :param lane_class_preds:
        :type lane_class_preds:
        :return: lane set
        :rtype: dict
        """
        predict_type = predict_type.detach().cpu().numpy()
        predict_loc = predict_loc.detach().cpu().numpy()
        if lane_class_preds is not None:
            lane_class_preds = lane_class_preds.detach().cpu().numpy()
        ...
        for h in range(self.feature_height):
            for w in range(self.feature_width):
                ...
                if self.with_lane_cls:
                    predict_lane_cls = lane_class_preds[index]
                    predict_lane_cls_name = self.decode_lane_type(predict_lane_cls)
                else:
                    predict_lane_cls_name = "line-single-solid-white"
                ...
                if up_lane.size + down_lane.size >= 2:
                    ...
                    lane_predict = Lane(prob,
                                        ...
                                        1,
                                        lane,
                                        lane_class_type=predict_lane_cls_name)
